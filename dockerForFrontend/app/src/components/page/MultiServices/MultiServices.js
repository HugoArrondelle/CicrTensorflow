import React, { useState, Fragment } from "react";
import { SaveButton } from './../../SaveButton/SaveButton';
import LoadingScreen from 'react-loading-screen';
import logo from './../../../image/cicr_logo.png';
import './MultiServices.css';

const API = process.env.REACT_APP_API;

export const MultiServices = () => {

    const [eyes, setShowEyes] = useState(false);
    const [nose, setShowNose] = useState(false);
    const [moustacheBeard, setShowMoustacheBeard] = useState(false);
    const [align, setShowAlign] = useState(false);
    const [background, setShowBackground] = useState(false);
    const [skin, setShowSkin] = useState(false);
    const [teeth, setShowTeeth] = useState(false);
    const [gray, setShowGray] = useState(false);

    const [urlImageTransform, setUrlImageTransform] = useState("");
    const [fileTest, setFileTest] = useState(null);

    const [loading, setLoading] = useState(false);
    let [previewImage, setpreviewImage] = useState(null);
    const [modalShow, setModalShow] = useState(false);

    const selectFile = async (event) => {
      setFileTest(event.target.files[0]);
      setpreviewImage(URL.createObjectURL(event.target.files[0]));
    };

    const handleUploadImage = async (event) =>  {
      event.preventDefault();

      const trueInputImg = document.querySelector('.trueInputImg');
      console.log("@@@@@@@@@@@ truInput: ", trueInputImg);

      if (trueInputImg.files.length) {
        setLoading(true);
      }
      else
        alert("You must insert an image!")
      const d = new FormData();
      d.append('file', fileTest);

      fetch(`${API}/multiService`, { method: 'POST', body: d, headers: {
        'algo' : [(eyes && "eyes"), (align && "align") , (background && "background"),( gray && "gray")]
        },
      })
      .then((response) => {
        response.json().then((body) => {
          setUrlImageTransform(body._id);
        });
        setLoading(false);
      });

    }

    return (
    <Fragment>
      {modalShow && (
        <div className="mobal_custom">
          <div className="modal__locked">
            <div className="modal-body">
              <center>
                <strong>Exemple</strong>
                <p>
                  This offer is not valid for your plan. Please upgrade your account to
                  enjoy more features.
                </p>
              </center>
            </div>
            <div className="modal-buttons">
              <button className="btn btn-footer btn-white" onClick={() => setModalShow(false)}>Cancel</button>
            </div>
          </div>
        </div>
      )}
      {(
              loading === true
              ? <LoadingScreen
                  loading={true}
                  bgColor='rgba(241,241,241,0.95)'
                  spinnerColor='rgba(205,43,57,1)'
                  textColor='#676767'
                  logoSrc={logo}
                  text='Algorithme en cours de traitement'
                >

                </LoadingScreen>
              :
              <div>
      <section className="main-home" id="home">
          <div className="home-page-photo"></div>
          <div className="home__header-content">
              <h1 className="intro-title">PHOTO EDITING ALGORITHM</h1>
              <p className="intro-text">Lorem ipsum dolor sit amet consectetur adictum piscing elit fusce sit amet inerdum neque ultra icies <br/><br/>  </p>
              <a className="btn-custom" href="/home">Get started</a>
          </div>
      </section>
      <div className="position-relative mobalIcon">
              <div className="position-absolute">
                  <button className='btn btn__Vid' onClick={() => setModalShow(true)}  >
                      <svg id="SVG7" enableBackground="new 0 0 512 512" height="50" width="60" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                          <g>
                          <path d="M446.25,0h-357c-28.05,0-51,22.95-51,51v357c0,28.05,22.95,51,51,51h102l76.5,76.5l76.5-76.5h102c28.05,0,51-22.95,51-51
                              V51C497.25,22.95,474.3,0,446.25,0z M293.25,408h-51v-51h51V408z M346.8,211.65l-22.95,22.95c-20.399,17.85-30.6,33.15-30.6,71.4
                              h-51v-12.75c0-28.05,10.2-53.55,30.6-71.4l30.601-33.15c10.2-7.65,15.3-20.4,15.3-35.7c0-28.05-22.95-51-51-51s-51,22.95-51,51
                              h-51c0-56.1,45.9-102,102-102c56.1,0,102,45.9,102,102C369.75,175.95,359.55,196.35,346.8,211.65z"/>
                          </g>
                      </svg>
                  </button>
              </div>

          </div>
          <section id="services">
              <div className="container">
          <div className="row">
                      <div className="col-md-12">
                          <h3 className="title text-center">Services</h3>
                          <div className="titleHR"><span></span></div>
                      </div>
                  </div>
      <div className="bg-color-sky-light">
          <div className="content-lg container">
              <div className="row justify-content-center mb-100">

                  <div className="row1 col-md-5 col-sm-6 md-margin-b-60">

                      <div className="  margin-t-50 margin-b-30">
                          <label>
                            <input

                              type="checkbox"
                              className="checkbox-custom"
                              data-testid="cbShowHide"
                              checked={eyes}
                              onChange={e => setShowEyes(e.target.checked)} />
                              <span  > Eyes <svg id="SVG5" enableBackground="0 0 511.999 511.999" height="25" viewBox="0 0 512 512" width="25" xmlns="http://www.w3.org/2000/svg"><g><path d="M508.745,246.041c-4.574-6.257-113.557-153.206-252.748-153.206S7.818,239.784,3.249,246.035
                              c-4.332,5.936-4.332,13.987,0,19.923c4.569,6.257,113.557,153.206,252.748,153.206s248.174-146.95,252.748-153.201
                              C513.083,260.028,513.083,251.971,508.745,246.041z M255.997,385.406c-102.529,0-191.33-97.533-217.617-129.418
                              c26.253-31.913,114.868-129.395,217.617-129.395c102.524,0,191.319,97.516,217.617,129.418
                              C447.361,287.923,358.746,385.406,255.997,385.406z"/><path d="M255.997,154.725c-55.842,0-101.275,45.433-101.275,101.275s45.433,101.275,101.275,101.275
                              s101.275-45.433,101.275-101.275S311.839,154.725,255.997,154.725z M255.997,323.516c-37.23,0-67.516-30.287-67.516-67.516
                              s30.287-67.516,67.516-67.516s67.516,30.287,67.516,67.516S293.227,323.516,255.997,323.516z"/>
                              </g></svg></span>
                          </label>
                      </div>
                          <div className="margin-t-50 margin-b-30">

                          <label>
                            <input
                              type="checkbox"
                              data-testid="cbShowHide"
                              checked={nose}
                              onChange={e => setShowNose(e.target.checked)} />
                            <span> Nose <svg id="SVG1" height="25" viewBox="0 0 24 24" width="25" xmlns="http://www.w3.org/2000/svg"><path d="m17.504 23.998c-1.313 0-2.567-.566-3.436-1.595-.179-.211-.151-.526.06-.705.211-.177.526-.15.704.06.861 1.021 2.213 1.454 3.53 1.137 1.229-.286 2.245-1.297 2.53-2.518.425-1.767-.476-3.506-2.141-4.139-.845-.319-1.441-1.103-1.558-2.046-.524-4.562-1.413-8.24-2.796-11.58-.39-.966-1.353-1.612-2.397-1.612s-2.007.646-2.397 1.608c-1.384 3.343-2.273 7.021-2.796 11.579-.117.947-.714 1.731-1.56 2.051-1.664.632-2.564 2.372-2.141 4.136.286 1.223 1.302 2.234 2.526 2.52 1.323.319 2.675-.116 3.535-1.136.179-.21.494-.236.704-.06.211.178.238.494.06.705-1.106 1.313-2.842 1.874-4.53 1.463-1.607-.374-2.892-1.654-3.269-3.262-.542-2.257.618-4.487 2.76-5.301.498-.188.851-.661.921-1.234.535-4.649 1.445-8.412 2.865-11.84.54-1.332 1.876-2.229 3.322-2.229s2.782.897 3.323 2.232c1.419 3.425 2.329 7.188 2.864 11.841.069.569.422 1.042.919 1.229 2.143.814 3.303 3.045 2.76 5.305-.376 1.604-1.66 2.885-3.272 3.26-.364.088-.729.131-1.09.131z"/><path d="m12 24c-1.107 0-2.135-.55-2.748-1.471l-.336-.504c-.428-.642-1.145-1.025-1.916-1.025h-.5c-.276 0-.5-.224-.5-.5s.224-.5.5-.5h.5c1.107 0 2.135.55 2.748 1.471l.336.504c.428.642 1.145 1.025 1.916 1.025s1.488-.383 1.916-1.025l.336-.504c.613-.921 1.641-1.471 2.748-1.471h.5c.276 0 .5.224.5.5s-.224.5-.5.5h-.5c-.771 0-1.488.383-1.916 1.025l-.336.504c-.613.921-1.641 1.471-2.748 1.471z"/></svg>

                          </span>
                          </label>



                              </div>

                              <div className="margin-t-50 margin-b-30">

                              <label>
                                <input
                                  type="checkbox"
                                  data-testid="cbShowHide"
                                  checked={moustacheBeard}
                                  onChange={e => setShowMoustacheBeard(e.target.checked)} />
                                <span> Moustache & Beard <svg id="SVG4" enableBackground="new 0 0 512 512" height="25" viewBox="0 0 512 512" width="25" xmlns="http://www.w3.org/2000/svg"><g><path d="m166 150c38.115 0 69.829-9.185 90-25.649 20.171 16.464 51.885 25.649 90 25.649 86.867 0 166.027-22.108 165.707-108.662-.029-5.903-3.516-11.265-8.921-13.652-5.361-2.358-11.704-1.406-16.113 2.563-33.721 30.366-62.787 22.119-105.077-7.471-22.237-15.571-43.242-22.778-65.596-22.778-28.359 0-47.886 15.513-60 25.752-12.114-10.239-31.641-25.752-60-25.752-21.768 0-42.686 6.958-64.819 22.266-42.95 29.677-72.411 38.042-105.854 7.983-4.395-3.97-10.723-4.937-16.113-2.563-5.405 2.388-8.892 7.749-8.921 13.652-.319 86.437 78.653 108.662 165.707 108.662z"/><path d="m482 151.13c-23.324 14-54.329 22.718-91.214 26.539.02.786.214 1.544.214 2.331 0 49.629-40.371 90-90 90h-90c-49.629 0-90-40.371-90-90 0-.791.211-1.542.231-2.329-36.892-3.822-67.903-12.541-91.231-26.549v65.469c0 135.264 91.835 257.688 221.884 294.837 1.347.382 2.724.572 4.116.572s2.769-.19 4.116-.571c130.049-37.149 221.884-159.574 221.884-294.837z"/><path d="m301 240c33.091 0 60-26.909 60-60 0-.22-.059-.432-.06-.652-5.065.158-9.646.652-14.94.652-35.259 0-65.903-6.782-90-19.79-24.097 13.008-54.741 19.79-90 19.79-5.292 0-9.871-.494-14.936-.652-.002.222-.064.43-.064.652 0 33.091 26.909 60 60 60z"/></g></svg>

                              </span>
                              </label>



                                  </div>

                                  <div className="margin-t-50 margin-b-30">

                                  <label>
                                    <input
                                      type="checkbox"
                                      data-testid="cbShowHide"
                                      checked={align}
                                      onChange={e => setShowAlign(e.target.checked)} />
                                    <span> ALIGN <svg id="SVG8" enableBackground="0 0 318.263 318.263" height="25" viewBox="0 0 318.263 318.263" width="25" xmlns="http://www.w3.org/2000/svg"><g><path d="M232.344,127.31c-1.664,0.052-3.196,0.866-4.163,2.204l-7.533,10.321c-6.451-33.851-29.496-58.723-56.427-58.723
                          c-29.607,0-52.065,23.121-57.002,56.691l-6.685-8.457c-1.195-1.499-3.066-2.268-4.985-1.984c-1.897,0.275-3.498,1.539-4.217,3.312
                          L80.67,157.129c-0.581,1.443-0.509,3.076,0.205,4.463l8.293,16.188c0.701,1.371,1.953,2.373,3.451,2.758l16.417,4.215
                          c6.515,31.958,27.888,52.41,55.195,52.41c25.125,0,46.919-20.851,55.029-52.263l21.434-4.306c1.621-0.335,3.008-1.392,3.743-2.866
                          l10.649-21.312c0.972-1.912,0.688-4.213-0.676-5.856l-17.765-21.321C235.593,127.966,234.071,127.213,232.344,127.31z"/>
                        <path d="M162.453,308.895c59.023,0,112.662-35.144,136.632-88.425l9.292,14.246c1.028,1.58,2.75,2.441,4.508,2.441
                          c1.014,0,2.026-0.29,2.93-0.872c2.493-1.623,3.202-4.95,1.568-7.438l-14.865-22.803c-0.052-0.086-0.143-0.117-0.199-0.2
                          c-0.127-0.169-0.268-0.335-0.421-0.488c-0.177-0.194-0.361-0.351-0.571-0.53c-0.174-0.137-0.341-0.283-0.54-0.399
                          c-0.189-0.114-0.374-0.189-0.573-0.283c-0.12-0.067-0.221-0.158-0.341-0.21c-0.115-0.042-0.252-0.021-0.374-0.062
                          c-0.192-0.062-0.393-0.116-0.603-0.158c-0.221-0.042-0.441-0.031-0.673-0.042c-0.262-0.01-0.52-0.01-0.787,0.021
                          c-0.215,0.021-0.409,0.041-0.618,0.105c-0.101,0.021-0.184-0.005-0.29,0.021l-36.063,10.445c-2.834,0.83-4.493,3.801-3.68,6.655
                          c0.683,2.358,2.83,3.885,5.165,3.885c0.504,0,1.008-0.072,1.498-0.21l25.328-7.337c-22.505,48.802-71.961,80.897-126.312,80.897
                          c-57.933,0-110.348-36.489-130.445-90.819c-1.021-2.793-4.123-4.21-6.903-3.182c-2.791,1.019-4.207,4.126-3.176,6.908
                          C43.574,269.582,100.043,308.895,162.453,308.895z"/>
                        <path d="M293.153,111.635c0.787,2.181,2.854,3.543,5.049,3.543c0.604,0,1.229-0.11,1.838-0.328
                          c2.792-1.016,4.231-4.102,3.213-6.893C281.818,48.989,225.236,9.369,162.458,9.369c-59.668,0-113.792,35.971-137.375,90.246
                          L9.178,83.708c-2.102-2.1-5.499-2.1-7.601,0c-2.102,2.103-2.102,5.502,0,7.602l21.588,21.588c0.021,0.021,0.052,0.035,0.076,0.056
                          c0.196,0.191,0.473,0.31,0.703,0.473c0.318,0.225,0.606,0.482,0.974,0.634c0.037,0.016,0.079,0.021,0.118,0.037
                          c0.026,0.01,0.042,0.031,0.068,0.042c0.271,0.102,0.538,0.076,0.806,0.128c0.357,0.071,0.701,0.205,1.055,0.205h0.242
                          c0,0,0,0,0.005,0h38.116c2.973,0,5.375-2.407,5.375-5.375c0-2.966-2.402-5.375-5.375-5.375H35.013
                          C56.932,53.43,107.14,20.12,162.453,20.12C220.731,20.12,273.246,56.887,293.153,111.635z"/></g></svg>

                                  </span>
                                  </label>



                                      </div>
                                      <div className="margin-t-50 margin-b-30">

                                      <label>
                                        <input
                                          type="checkbox"
                                          data-testid="cbShowHide"
                                          checked={background}
                                          onChange={e => setShowBackground(e.target.checked)} />
                                        <span> BACKGROUND <svg id="SVG7" enableBackground="0 0 512 512" height="25" viewBox="0 0 578.405 578.405" width="25" xmlns="http://www.w3.org/2000/svg"><g><path d="M491.54,352.312c11.024,0,19.961-8.937,19.961-19.961V202.604c0-44.026-35.818-79.844-79.844-79.844h-41.918V79.844
                              C389.739,35.818,353.921,0,309.895,0H80.343C36.317,0,0.499,35.818,0.499,79.844v229.552c0,44.026,35.818,79.844,79.844,79.844
                              h41.918v42.916c0,44.026,35.818,79.844,79.844,79.844h229.552c44.026,0,79.844-35.818,79.844-79.844
                              c0-11.024-8.937-19.961-19.961-19.961c-11.024,0-19.961,8.937-19.961,19.961c0,22.013-17.909,39.922-39.922,39.922H202.105
                              c-22.013,0-39.922-17.909-39.922-39.922V202.604c0-22.013,17.909-39.922,39.922-39.922h229.552
                              c22.013,0,39.922,17.909,39.922,39.922v129.747C471.579,343.375,480.516,352.312,491.54,352.312z M40.421,79.844
                              c0-22.013,17.909-39.922,39.922-39.922h28.358l-68.28,68.273V79.844z M122.261,349.318H83.482l38.779-38.779V349.318z
                               M122.261,202.604v51.474l-76.033,76.034c-3.684-6.043-5.808-13.136-5.808-20.717v-31.354l82.117-82.118
                              C122.355,198.127,122.261,200.355,122.261,202.604z M40.421,221.583v-56.932L165.02,40.065c0.047-0.047,0.094-0.095,0.141-0.143
                              h56.921L40.421,221.583z M253.587,122.76h-51.481c-2.25,0-4.477,0.094-6.68,0.276l78.182-78.182
                              c1.49-1.49,2.695-3.157,3.615-4.932h32.672c7.58,0,14.673,2.124,20.717,5.808l-76.749,76.749
                              C253.769,122.572,253.678,122.666,253.587,122.76z M349.817,122.76h-39.777l39.777-39.777V122.76z"/></g></svg>

                                      </span>
                                      </label>



                                          </div>
                                          <div className="margin-t-50 margin-b-30">

                                          <label>
                                            <input
                                              type="checkbox"
                                              data-testid="cbShowHide"
                                              checked={skin}
                                              onChange={e => setShowSkin(e.target.checked)} />
                                            <span> SKIN <svg id="SVG2" enableBackground="new 0 0 512 512" height="25" viewBox="0 0 512 512" width="25" xmlns="http://www.w3.org/2000/svg"><g><path d="m457.643 486.394-54.463-54.464c15.73-22.59 19.245-51.162 10.504-76.416 23.285-26.791 36.004-60.687 36.004-96.515 0-75.579-42.651-141.373-105.141-174.61.132-.801.218-1.618.218-2.457-.001-45.177-42.59-81.932-94.94-81.932s-94.939 36.755-94.939 81.933c0 .839.086 1.655.218 2.457-62.49 33.237-105.141 99.031-105.141 174.61 0 47.823 22.79 91.957 61.351 119.658 2.785 73.997 63.843 133.342 138.511 133.342 39.488 0 76.737-16.672 103.041-45.91 10.258-2.39 20.146-6.711 29.1-12.947l54.464 54.464c2.929 2.929 6.767 4.394 10.606 4.394s7.678-1.464 10.607-4.394c5.859-5.858 5.859-15.356 0-21.213zm-163.04-64.114c-21.425-21.426-21.425-56.288 0-77.713 10.713-10.713 24.785-16.069 38.857-16.069s28.145 5.357 38.857 16.069c21.425 21.426 21.425 56.288 0 77.713s-56.287 21.425-77.714 0zm-44.778-392.28c31.163 0 57.26 17.648 63.508 41.102-19.303-6.311-39.898-9.738-61.282-9.738h-4.453c-21.384 0-41.979 3.427-61.282 9.738 6.249-23.454 32.346-41.102 63.509-41.102zm-169.862 228.999c0-92.434 75.201-167.636 167.636-167.636h4.453c92.435 0 167.636 75.201 167.636 167.636 0 24.992-7.774 48.803-22.164 68.668-1.28-1.468-2.596-2.915-3.993-4.313 0 0 0 0 0-.001-1.641-1.641-3.345-3.176-5.083-4.654v-34.641c0-56.55-46.007-102.558-102.558-102.558h-72.128c-56.551 0-102.558 46.007-102.558 102.558v54.723c-19.908-21.425-31.241-49.689-31.241-79.782zm61.24 114.379v-89.32c0-40.008 32.549-72.558 72.558-72.558h72.128c40.009 0 72.558 32.549 72.558 72.558v18.201c-29.103-8.906-62.077-1.884-85.057 21.095-24.129 24.129-30.659 59.275-19.634 89.39-1.295.172-2.606.27-3.932.27-10.568 0-20.198-5.59-25.129-14.589-3.982-7.266-13.099-9.925-20.363-5.945-7.265 3.982-9.927 13.099-5.945 20.363 10.2 18.61 29.909 30.171 51.438 30.171 6.864 0 13.583-1.174 19.918-3.43 1.173 1.329 2.377 2.639 3.648 3.91 10.208 10.208 22.392 17.258 35.331 21.174-17.354 11.211-37.716 17.332-58.897 17.332-59.894 0-108.622-48.728-108.622-108.622z"/></g></svg>


                                          </span>
                                          </label>


                                              </div>
                                              <div className="margin-t-50 margin-b-30">

                                              <label>
                                                <input
                                                  type="checkbox"
                                                  data-testid="cbShowHide"
                                                  checked={teeth}
                                                  onChange={e => setShowTeeth(e.target.checked)} />
                                                <span> TEETH <svg id="SVG3" enableBackground="new 0 0 511.774 511.774" height="25" viewBox="0 0 511.774 511.774" width="25" xmlns="http://www.w3.org/2000/svg"><path d="m475.746 297.696c-27.857 0-28.092-.455-31.884 1.368-3.051-2.649-6.928-4.244-11.149-4.244h-34.513c-3.765 0-7.253 1.272-10.13 3.425-3.052-2.078-6.735-3.295-10.697-3.295-42.321 0-42.624-.812-48.412 2.44-3.27-2.906-7.56-4.688-12.269-4.688h-50.293c-3.902 0-7.521 1.221-10.509 3.291-2.988-2.07-6.606-3.291-10.509-3.291h-50.293c-4.709 0-8.999 1.782-12.269 4.688-5.788-3.252-6.091-2.44-48.412-2.44-3.963 0-7.645 1.217-10.697 3.295-2.876-2.152-6.365-3.425-10.13-3.425h-34.512c-4.221 0-8.098 1.595-11.149 4.244-3.814-1.834-4.111-1.368-31.884-1.368-8.456 0-15.336 7.825-15.336 17.443 0 23.08-1.05 29.947 4.132 40.179-.681 39.379 19.564 82.488 78.71 87.766 23.313 2.08 51.462.158 81.261-1.877 62.764-4.284 79.36-4.288 142.176 0 25.692 1.755 56.73 4.066 81.261 1.877 59.146-5.278 79.391-48.387 78.71-87.766 5.203-10.273 4.132-17.508 4.132-40.179 0-9.617-6.879-17.443-15.335-17.443zm-79.31 16.318c0-2.166 1.158-3.193 1.764-3.193h34.513c.702 0 1.76 1.222 1.764 3.063.06 30.245.384 33.896-2.02 39.566-9.111 21.597-36.02 13.399-36.02-10.125v-29.311zm-61.235 0c0-1.689 1.374-3.063 3.063-3.063h39.108c1.689 0 3.067 1.374 3.063 3.063-.065 28.526.328 29.928-.421 33.772-5.311 27.421-44.815 22.258-44.815-4.331v-29.441zm-71.311-2.802c0-1.384 1.125-2.51 2.509-2.51h50.293c1.384 0 2.509 1.126 2.509 2.51v46.259c0 11.625-9.458 21.083-21.083 21.083h-13.145c-11.625 0-21.083-9.458-21.083-21.083zm-71.31 0c0-1.384 1.125-2.51 2.509-2.51h50.293c1.384 0 2.509 1.126 2.509 2.51v46.259c0 11.625-9.458 21.083-21.083 21.083h-13.145c-11.625 0-21.083-9.458-21.083-21.083 0-24.465 0-32.958 0-46.259zm-61.236 32.113v-29.312c0-1.689 1.374-3.063 3.063-3.063h39.108c1.689 0 3.063 1.374 3.063 3.063v29.441c.002 30.734-45.234 29.256-45.234-.129zm-54.04-29.441c0-1.842 1.062-3.063 1.764-3.063h34.513c.702 0 1.764 1.222 1.764 3.063v29.441c0 30.424-38.04 29.492-38.04 0-.001-17.992-.001-13.427-.001-29.441zm-37.566 35.458c-4.014-6.824-3.039-12.099-3.039-34.202 0-.655.144-1.14.279-1.443h24.047c.399.892.279-.215.279 23.881 0 15.658-13.594 25.113-21.566 11.764zm367.079 77.806c-22.061 1.972-49.597.089-78.749-1.902-63.513-4.336-80.814-4.338-144.355 0-29.151 1.992-56.688 3.876-78.749 1.902-37.088-3.309-58.024-23.505-63.075-56.527.101.003 13.1 5.146 24.955-6.437 12.853 22.04 41.774 24.004 56.848 3.242 13.694 17.269 39.267 19.724 55.969 4.841 5.82 13.311 19.089 22.515 34.003 22.287 12.579-.192 28.767 2.868 42.228-14.101 14.765 18.612 35.756 13.155 42.228 14.101 14.578 0 28.012-8.621 34.003-22.287 16.699 14.88 42.248 12.446 55.969-4.841 14.899 20.519 43.826 19.088 56.849-3.242 11.85 11.577 24.851 6.44 24.954 6.437-5.049 32.986-25.953 53.214-63.078 56.527zm68.264-89.571c0 9.001-5.685 20.037-14.286 17.866-6.522-1.701-10.32-10.115-10.32-17.866 0-24.257-.119-22.989.279-23.881h24.047c.4.893.28-.214.28 23.881z"/><path d="m366.82 86.845c3.847 0 7.115-2.732 7.84-6.439 1.034-4.776-2.709-9.561-7.84-9.561-3.844 0-7.114 2.724-7.84 6.44-1.025 4.736 2.662 9.56 7.84 9.56z"/><path d="m479.927 99.149c-25.049-4.153-53.069-14.472-82.915-22.072-4.281-1.087-8.636 1.498-9.727 5.779-1.09 4.281 1.497 8.636 5.779 9.727 29.534 7.52 57.874 17.979 84.245 22.352 8.054 1.335 13.303 4.765 16.046 10.482 6.009 12.528.518 33.909-6.303 51.392-8.816-18.015-29.82-22.641-43.474-7.521-14.416-21.336-43.256-20.575-56.864.946-14.705-16.563-40.067-17.396-55.816-2.072-6.461-12.139-19.063-19.758-32.781-19.758-6.396.917-27.549-4.468-42.228 14.101-7.029-8.892-17.788-14.114-29.083-14.101-12.306.014-14.702-.169-18.519.395-11.947 1.738-21.997 9.201-27.406 19.363-15.785-15.358-41.091-14.47-55.816 2.072-13.68-21.635-42.51-22.191-56.864-.946-13.559-15.016-34.395-10.634-43.454 7.483-5.492-14.011-12.772-37.905-6.323-51.353 2.743-5.72 7.992-9.148 16.047-10.484 49.238-8.165 104.179-37.117 163.012-29.774 74.142 9.253 110.064-1.62 138.863-1.152 4.391.105 8.057-3.452 8.128-7.87.338-20.963-44.093 5.743-145.009-6.854-62.582-7.813-120.259 22.015-167.612 29.865-47.177 7.824-34.032 64.923-11.154 107.045v20.398c0 9.618 6.88 17.443 15.336 17.443 28.503 0 26.983.156 29.299-.437 3.26 4.264 8.205 6.991 13.734 6.991h34.513c4.264 0 8.181-1.624 11.246-4.321 10.316 9.555 19.824 7.87 38.23 7.87 6.79 0 13.047-2.31 18.04-6.177 4.849 7.287 13.134 12.102 22.525 12.102h33.229c7.42 0 14.15-3.006 19.041-7.862 4.891 4.856 11.621 7.862 19.041 7.862h33.229c9.391 0 17.675-4.814 22.525-12.102 10.037 7.774 18.976 6.177 36.228 6.177 7.73 0 14.773-2.99 20.041-7.87 3.065 2.698 6.982 4.321 11.246 4.321h34.513c5.529 0 10.474-2.727 13.734-6.991 3.081.789 3.243.364 29.299.437 8.456.024 15.336-7.825 15.336-17.443v-20.325c22.654-42.459 36.218-99.256-11.157-107.118zm-418.903 128.887h-24.046c-.34-.76-.28-33.584-.279-33.689.042-9.83 5.634-18.135 12.303-18.135s12.303 8.305 12.303 18.135c-.001 35.308.115 32.81-.281 33.689zm54.32 3.489c0 1.842-1.062 3.064-1.764 3.064h-34.512c-.702 0-1.764-1.223-1.764-3.064 0-9.911 0-29.446 0-39.249 0-12.471 8.532-22.617 19.02-22.617s19.021 10.146 19.021 22.617c-.001 13.2-.001 24.869-.001 39.249zm61.236-6.91c0 7.457-6.066 13.523-13.523 13.523h-18.188c-7.457 0-13.523-6.066-13.523-13.523v-28.79c0-12.511 10.201-22.617 22.618-22.617 12.471 0 22.618 10.146 22.618 22.617v28.79zm71.31 8.408c0 6.088-4.953 11.041-11.041 11.041h-33.229c-6.088 0-11.041-4.953-11.041-11.041 0-13.794 0-22.928 0-47.535 0-11.622 9.455-21.082 21.083-21.082h13.145c11.645 0 21.083 9.476 21.083 21.082zm71.311 0c0 6.088-4.953 11.041-11.041 11.041h-33.229c-6.088 0-11.041-4.953-11.041-11.041v-47.535c0-11.6 9.433-21.082 21.083-21.082h13.145c11.645 0 21.083 9.476 21.083 21.082zm61.235-8.408c0 7.457-6.066 13.523-13.523 13.523h-18.188c-7.457 0-13.523-6.066-13.523-13.523v-28.79c0-12.471 10.146-22.617 22.618-22.617 12.422 0 22.618 10.112 22.618 22.617v28.79zm54.04 6.91c0 1.842-1.062 3.064-1.764 3.064h-34.512c-.702 0-1.764-1.223-1.764-3.064 0-8.117 0-25.126 0-39.249 0-12.471 8.532-22.617 19.021-22.617 10.487 0 19.02 10.146 19.02 22.617-.001 18.31-.001 30.045-.001 39.249zm40.326-3.489h-24.046c-.4-.889-.28 1.473-.28-33.689 0-9.83 5.634-18.135 12.303-18.135s12.303 8.305 12.303 18.135c-.001 35.11.116 32.804-.28 33.689z"/></svg>

                                              </span>
                                              </label>



                                                  </div>
                                                  <div className="margin-t-50 margin-b-30">

                                                  <label>
                                                    <input
                                                      type="checkbox"
                                                      data-testid="cbShowHide"
                                                      checked={gray}
                                                      onChange={e => setShowGray(e.target.checked)} />
                                                    <span> GREY <svg id="SVG6" enableBackground="0 0 578.405 578.405" height="25" viewBox="0 0 578.405 578.405" width="25" xmlns="http://www.w3.org/2000/svg"><g><path d="M289.203,0C129.736,0,0,129.736,0,289.203C0,448.67,129.736,578.405,289.203,578.405
                                          c159.467,0,289.202-129.735,289.202-289.202C578.405,129.736,448.67,0,289.203,0z M28.56,289.202
                                          C28.56,145.48,145.481,28.56,289.203,28.56l0,0v521.286l0,0C145.485,549.846,28.56,432.925,28.56,289.202z"/></g></svg>

                                                  </span>
                                                  </label>



                                                      </div>
                  </div>
                    <div className="row1 col-md-5 col-sm-6 md-margin-b-60 flex-custom align-items-center justify-content-center">
                          <div className="title ">
                              Upload your image
                          </div>
                          <div className="input-group mb-3">
                          <form onSubmit={handleUploadImage}>
                              <input className="trueInputImg" name="file" type="file" onChange={selectFile}  />
                              <button>Upload</button>
                          </form>
                          </div>
                          </div>
                  <section id="services">
              <div className="container">
              <div className="row">
                      <div className="col-md-12 services-item">
                          <h3 className="title text-center">Upload your image</h3>
                          <div className="titleHR"><span></span></div>
                      </div>
                  </div>
                  <div className="row">

                      <div className="imageCase">
                          <div className="mt-5 text-center text-uppercase">Select your image</div>
                          <div className="card-body">
                              <div className="card-title text-center col d-flex align-items-center justify-content-center">

                              {(
                                      previewImage === null
                                      ? <svg id="SVGImageCase" height="50%" viewBox="0 -21 511.98744 511" width="50%" xmlns="http://www.w3.org/2000/svg"><path d="m377.652344 469.828125c-4.03125 0-8.148438-.511719-12.226563-1.578125l-329.898437-88.34375c-25.449219-7.019531-40.617188-33.34375-33.960938-58.773438l36.265625-139.070312c2.21875-8.535156 10.945313-13.71875 19.519531-11.433594 8.535157 2.21875 13.675782 10.964844 11.4375 19.519532l-36.269531 139.09375c-2.238281 8.574218 2.859375 17.425781 11.394531 19.773437l329.707032 88.300781c8.46875 2.238282 17.214844-2.796875 19.433594-11.222656l11.261718-45.441406c2.136719-8.574219 10.796875-13.78125 19.371094-11.6875 8.578125 2.132812 13.804688 10.792968 11.691406 19.367187l-11.304687 45.65625c-5.699219 21.609375-25.152344 35.839844-46.421875 35.839844zm0 0"/><path d="m463.988281 341.828125h-330.667969c-26.472656 0-48-21.527344-48-48v-245.335937c0-26.472657 21.527344-48 48-48h330.667969c26.472657 0 48 21.527343 48 48v245.335937c0 26.472656-21.527343 48-48 48zm-330.667969-309.335937c-8.832031 0-16 7.167968-16 16v245.335937c0 8.832031 7.167969 16 16 16h330.667969c8.832031 0 16-7.167969 16-16v-245.335937c0-8.832032-7.167969-16-16-16zm0 0"/><path d="m191.988281 149.828125c-23.53125 0-42.667969-19.136719-42.667969-42.667969s19.136719-42.667968 42.667969-42.667968 42.664063 19.136718 42.664063 42.667968-19.132813 42.667969-42.664063 42.667969zm0-53.335937c-5.890625 0-10.667969 4.78125-10.667969 10.667968 0 5.886719 4.777344 10.667969 10.667969 10.667969 5.886719 0 10.664063-4.78125 10.664063-10.667969 0-5.886718-4.777344-10.667968-10.664063-10.667968zm0 0"/><path d="m101.746094 320.066406c-4.09375 0-8.191406-1.558594-11.304688-4.691406-6.25-6.25-6.25-16.386719 0-22.636719l96.425782-96.425781c14.59375-14.59375 38.335937-14.59375 52.90625 0l25.792968 25.792969 79.230469-95.105469c7.082031-8.492188 17.472656-13.398438 28.503906-13.460938 11.859375-.660156 21.460938 4.734376 28.605469 13.121094l106.199219 123.902344c5.757812 6.699219 4.96875 16.8125-1.730469 22.570312-6.71875 5.761719-16.808594 4.972657-22.570312-1.726562l-106.238282-123.945312c-1.410156-1.6875-3.136718-1.601563-4.097656-1.902344-.914062 0-2.6875.277344-4.09375 1.941406l-90.453125 108.589844c-2.882813 3.453125-7.082031 5.546875-11.5625 5.738281-4.566406.253906-8.875-1.496094-12.035156-4.671875l-38.183594-38.1875c-2.710937-2.710938-4.949219-2.710938-7.660156 0l-96.425781 96.40625c-3.117188 3.132812-7.210938 4.691406-11.308594 4.691406zm0 0"/></svg>
                                      : <img className="preview" src={previewImage} alt="" />
                              )}
                              </div>


                          </div>
                      </div>

                  </div>

              </div>

          </section>


              {urlImageTransform && (
                <div className="row">
                  <div className="container">
                      <div className="row">
                          <div className="imageCase">
                              <div className="mt-5 text-center text-uppercase">your results</div>
                              <div className="card-body">
                                  <div className="card-text col d-flex align-items-center justify-content-center">
                                            <img className="targetImg preview" src={urlImageTransform} alt="" />
                                  </div>
                              </div>
                          </div>
                          <div className="col-lg-5 col-md-5 col-sm-5 container justify-content-center services-item">
                          <SaveButton/>
                          </div>
                      </div>
                  </div>
                  </div>
                )}

          </div>


          {eyes &&   <div className="row">
              <div className="col-md-12">
                <h3 className="title text-center">EYES</h3>
                <div className="titleHR"><span></span></div>
                <div className="param">
                  <p>Param </p>
                </div>
              </div>
            </div>
          }
          {nose &&   <div className="row">
              <div className="col-md-12">
                <h3 className="title text-center">NOSE</h3>
                <div className="titleHR"><span></span></div>
                <div className="param">
                  <p>Param </p>
                </div>
              </div>
            </div>
          }
          {moustacheBeard &&   <div className="row">
              <div className="col-md-12">
                <h3 className="title text-center">MOUSTACHE & BEARD</h3>
                <div className="titleHR"><span></span></div>
                <div className="param">
                  <p>Param </p>
                </div>
              </div>
            </div>
          }
          {align &&   <div className="row">
              <div className="col-md-12">
                <h3 className="title text-center">ALIGN</h3>
                <div className="titleHR"><span></span></div>
                <div className="param">
                  <p>Param </p>
                </div>
              </div>
            </div>
          }
          {background &&   <div className="row">
              <div className="col-md-12">
                <h3 className="title text-center">BACKGROUND</h3>
                <div className="titleHR"><span></span></div>
                <div className="param">
                  <p>Param </p>
                </div>
              </div>
            </div>
          }
          {skin &&   <div className="row">
              <div className="col-md-12">
                <h3 className="title text-center">SKIN</h3>
                <div className="titleHR"><span></span></div>
                <div className="param">
                  <p>Param </p>
                </div>
              </div>
            </div>
          }
          {teeth &&   <div className="row">
              <div className="col-md-12">
                <h3 className="title text-center">TEETH</h3>
                <div className="titleHR"><span></span></div>
                <div className="param">
                  <p>Param </p>
                </div>
              </div>
            </div>
          }
          {gray &&   <div className="row">
              <div className="col-md-12">
                <h3 className="title text-center">GRAY</h3>
                <div className="titleHR"><span></span></div>
                <div className="param">
                  <p>Param </p>
                </div>
              </div>
            </div>
          }
      </div>
      </div>
      </div>
      </section>
      </div>
  )}
  </Fragment>
  );
}

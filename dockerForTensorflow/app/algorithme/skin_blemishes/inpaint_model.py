...
from algorithme.skin_blemishes.inpaint_ops import gen_conv, gen_deconv, dis_conv
from algorithme.skin_blemishes.inpaint_ops import random_bbox, bbox2mask, local_patch, brush_stroke_mask
from algorithme.skin_blemishes.inpaint_ops import resize_mask_like, contextual_attention
...
class InpaintCAModel(Model):
    ...
    def build_inpaint_net(self, x, mask, reuse=tf.AUTO_REUSE,
    ...
    def build_sn_patch_gan_discriminator(self, x, reuse=tf.AUTO_REUSE, training=True):
    ...
    def build_gan_discriminator(
            self, batch, reuse=tf.AUTO_REUSE, training=True):
    ...
    def build_server_graph(self, FLAGS, batch_data, reuse=tf.AUTO_REUSE, is_training=False):
        """
        """
        # generate mask, 1 represents masked point
        if FLAGS.guided:
